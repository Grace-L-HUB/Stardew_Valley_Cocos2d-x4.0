...
bool InventoryUI::init ( Inventory* inventory , std::string sceneName ) {    
    ...
    // 创建和注册事件观察者
    std::set<GameEventType> supportedEvents = {
        GameEventType::ITEM_ADDED,
        GameEventType::ITEM_REMOVED,
        GameEventType::ITEM_UPDATED,
        GameEventType::SELECTED_ITEM_CHANGED
    };
    
    _eventObserver = new UIEventObserver(this, 10);
    _eventObserver->on(GameEventType::ITEM_ADDED, [this](const GameEvent& event) {
        updateDisplay();
    })->on(GameEventType::ITEM_REMOVED, [this](const GameEvent& event) {
        updateDisplay();
    })->on(GameEventType::ITEM_UPDATED, [this](const GameEvent& event) {
        updateDisplay();
    })->on(GameEventType::SELECTED_ITEM_CHANGED, [this](const GameEvent& event) {
        updateDisplay();
    });
    
    EventManager::getInstance()->registerObserver(_eventObserver);
...
void InventoryUI::onExit() {
    // 取消注册事件观察者
    if (_eventObserver) {
        EventManager::getInstance()->unregisterObserver(_eventObserver);
